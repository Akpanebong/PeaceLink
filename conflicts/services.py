...
def send_referral_sms(stakeholder, referral):
    case = referral.case

    message = (
        f"New conflict referral {case.case_id}. "
        f"Type: {case.get_conflict_type_display()}. "
        f"Severity: {case.get_severity_display()}. "
        f"Please log into the system to review."
    )

    # SMS provider goes here.